...
describe("handlePlanRunCommand / handlePlanListCommand", () => {
	...
	it("runs an existing project plan by name", async () => {
		...
		vi.spyOn(session, "prompt").mockResolvedValue(undefined as any);
		...
		vi.spyOn(session, "setPlanReferencePath").mockImplementation(() => {});
		vi.spyOn(session, "markPlanReferenceSent").mockImplementation(() => {});
		vi.spyOn(session, "getActiveToolNames").mockReturnValue([]);
		...
		await (mode as any).handlePlanRunCommand?.("FOO_PLAN");
	...
	it("does NOT clear session when --keep is passed", async () => {
		...
		await (mode as any).handlePlanRunCommand?.("FOO_PLAN --keep");
	...
	it("shows error and does not prompt when plan is missing", async () => {
		...
		await (mode as any).handlePlanRunCommand?.("DOES_NOT_EXIST");
	...
	it("shows plan names when saved plans exist", async () => {
		...
		await (mode as any).handlePlanListCommand?.();
	...
	it("shows 'No approved plans' when no plans exist", async () => {
		const statusSpy = vi.spyOn(mode, "showStatus").mockImplementation(() => {});
		...
		expect(statusSpy).toHaveBeenCalled();
		const call = statusSpy.mock.calls[0]?.[0] ?? "";
		expect(call).toContain("No approved plans");
	});
});
