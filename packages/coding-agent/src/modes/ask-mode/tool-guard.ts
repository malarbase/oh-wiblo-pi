...
const ASK_MODE_BLOCKED_MESSAGE =
	'Cannot use write tools in Ask mode. Call switch_mode("agent") to request a mode switch, then retry.';
				...
				if (!check.allowed) {
					...
					return {
						content: [
							{
								type: "text",
								text: `Cannot use ${tool.name} in Ask mode: ${reason}. Call switch_mode("agent") to request a mode switch, then retry.`,
							},
						],
					};
				}
