...
import { Recovery, type RecoveryResult, tryRecoverHashlineWithCache } from "./recovery";
...
export interface PatcherOptions {
	...
	/**
	 * Optional per-session file-read cache used as a fallback when the snapshot
	 * store cannot recover a stale tag.
	 */
...
export class Patcher {
	...
	readonly fileReadCache: FileReadCache | undefined;
	constructor(options: PatcherOptions) {
		...
		this.fileReadCache = options.fileReadCache;
	}
	...
	#applyWithRecovery(args: {
		...
		if (this.fileReadCache) {
			const cacheRecovered = tryRecoverHashlineWithCache({
				cache: this.fileReadCache,
				absolutePath: canonicalPath,
				currentText: normalized,
				edits: resolved as unknown as import("./types").HashlineEdit[],
				options: {},
			});
			if (cacheRecovered) {
				return {
					text: cacheRecovered.lines,
					firstChangedLine: cacheRecovered.firstChangedLine,
					warnings: cacheRecovered.warnings,
				};
			}
		}
