...
import type { HashlineApplyOptions, HashlineApplyResult, HashlineEdit } from "./types";

/** OWP wrapper around applyEdits that uses the older edit shape. */
export function applyHashlineEdits(
	text: string,
	edits: readonly HashlineEdit[],
	_options?: HashlineApplyOptions,
): HashlineApplyResult {
	const result = applyEdits(text, edits as Edit[]);
	return {
		lines: result.text,
