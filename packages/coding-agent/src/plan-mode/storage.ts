...
/**
 * Write plan content to disk, handling both `local://` URLs and absolute paths.
 *
 * - `local://` URLs are resolved via the session's artifacts directory.
 * - Absolute paths are used directly (project storage at `.omp/plans/`).
 * - Parent directories are created as needed.
 *
 * This utility exists so callers in upstream-controlled files (e.g.
 * `interactive-mode.ts`) can delegate path resolution to fork-owned code,
 * reducing merge-conflict surface during rebases.
 */
export async function writePlanToDisk(
	planContent: string,
	targetPlanPath: string,
	ctx: { getArtifactsDir: () => string | null; getSessionId: () => string | null },
): Promise<string> {
	const normalized = normalizeLocalScheme(targetPlanPath);
	let resolvedPath: string;
	if (normalized.startsWith("local:")) {
		resolvedPath = resolveLocalUrlToPath(normalized, {
			getArtifactsDir: ctx.getArtifactsDir,
			getSessionId: ctx.getSessionId,
		});
	} else {
		resolvedPath = path.isAbsolute(targetPlanPath) ? targetPlanPath : path.resolve(targetPlanPath);
	}
	await fs.mkdir(path.dirname(resolvedPath), { recursive: true });
	await fs.writeFile(resolvedPath, planContent);
	return resolvedPath;
}
