/**
 * Filesystem paths for pi-peon (OWP-compatible fork).
 *
 * Two roots are involved:
 *
 *  1. **CESP pack root** — `~/.openpeon/packs/<pack>/` (overridable via
 *     `PEON_PACKS_DIR`). This is the location prescribed by the CESP spec.
 *
...
/** Per-extension state directory under the pi state tree. */
